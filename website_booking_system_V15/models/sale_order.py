...
class SaleOrderLine(models.Model):
    ...
    def _default_get_user(self):
        return self._context.get('uid')

    booking_time_slot = fields.Char(string="Booking Time Slot")
    booking_plan = fields.Char(string="Booking Plan")
    booking_date = fields.Char(string="Booking Date")
    user_id = fields.Many2one('res.users', string="Current User", default=_default_get_user)
...
def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
    ...
    product_record = self.env['product.template'].search([('id', '=', product_id)])
    if not product_record.is_available_booking:
        self.ensure_one()
        ...
        if line_id is not False:
            ...
            if kwargs:
                having_kwargs = True
            for order_line_rec in order_line_obj:
                if order_line_rec:
                    if order_line_rec.user_id.id == active_user_id.id and order_line_rec.product_id.id == product_record.id and order_line_rec.booking_time_slot == kwargs.get("book_time") and order_line_rec.booking_date == kwargs.get("book_date") and order_line_rec.booking_plan == kwargs.get("book_plan"):
                        exiting_order_line = True
                        exiting_order_line_id = order_line_rec

        # Create line if no line with product_id can be located
        if not order_line  or exiting_order_line != True and having_kwargs == True:
            exiting_order_line = False
            having_kwargs = False      
            if not product:
                raise UserError(_("The given product does not exist therefore it cannot be added to cart."))
            ...
            # create the line
            values['booking_time_slot'] = kwargs.get('book_time')
            values['booking_plan'] = kwargs.get('book_plan')
            values['booking_date'] = kwargs.get('book_date')
            values['temp_unit_price'] = kwargs.get('book_price')
            values['price_unit'] = kwargs.get('book_price')
        ...
        if set_qty:
            ...
            if exiting_order_line_id:
                quantity = exiting_order_line_id.product_uom_qty + (add_qty or 0)
            else:
                quantity = order_line.product_uom_qty + (add_qty or 0)
        ...
        if quantity <= 0:
            ...
            if exiting_order_line_id:
                exiting_order_line_id.write(values)
            else:
                values['price_unit'] = order_line.temp_unit_price
                order_line.write(values)
