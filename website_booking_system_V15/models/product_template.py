...
class Products(models.Model):
    ...
    booking_qty = fields.Integer(string="Max Booking Qty at a time")
    ...
    def get_seat_available(self, service_product_id):
        ...
        temp_qty = 0
        temp_product = self.search([('id', '=', temp.get('service_product_id'))])
        book_slot_record_time = self.env['booking.slot'].search([('id', '=', temp.get('booking_time'))])
        book_slot_record_plan = self.env['booking.slot'].search([('id', '=', temp.get('booking_plan'))])
